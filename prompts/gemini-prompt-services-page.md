...
TASK
════════════════════════════════════════
Build a complete, standalone services.html page for this site. It must be pixel-perfect consistent with the existing pages. The full design system, patterns, and content are provided below.
...
□ 2. No CSS framework other than Tailwind CDN — no Bootstrap, no custom framework
□ 3. No icon library imports — every icon is a hand-written inline SVG
□ 4. No localStorage usage anywhere in the JS
□ 5. No map library (no Leaflet, no Google Maps JS API)
□ 6. Fonts are only Barlow Condensed, Barlow, and Lato — no other fonts added
